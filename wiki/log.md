...
## [2026-05-01] decision | Phase 7a design — R_OP_TEXT + shared-strbuf

Coding-architect pass on phase 7a. Resolved: (Q1) `R_OP_TEXT=7` packs `a=color, b=col, c=row, x=strbuf-offset, y=len`; pixel coords derived as `b*8, c*8` against the real 160×192 type-2 screen — character-cell grid 20×24. (Q2) Strings live in a new 256 B `strbuf[]` inside `RenderQueue`; parent copies bytes in, child reads from same shared mapping — caller's source buffer needn't outlive the call. Allocation wraps on overflow with a `ren_flush()` guard. `RENDER_MAGIC` bumps `0x5244 → 0x5245` to fail-fast against stale `pocrndc`. (Q3) 96-entry 8×8 `g_font[][8]` baked `static const` in a new `font8x8.h`, included only by `poc_rndc.c` → 768 B in code segment, zero data-segment cost. (Q4) Module split: new `score.c` holds pure net-worth math + the three screen drivers (`summary_show`/`failed_show`/`victory_show`); `render.c` gains `ren_text(col,row,color,str)`; `poc_rndc.c` gains `draw_glyph` + `draw_string` reusing existing `putpx`. `main.c` rewires for mode-driven `max_rounds` (6/12/12), replaces the `ph_sum` stub, branches to colony-failure when `combined < $60000` in Standard/Tournament, and falls through to victory after the final round. New code estimate ~1.3 KB, stays well under budget. Handed to direct implementation.

---

## [2026-05-01] ingest | Phase 7a requirements (GDD §8, §17, §19.3.5/13/14)

Business-analyst pass for phase 7a (Summary / Scoring). Ingested GDD §8 (Summary content + duration), §17.1–17.4 (net-worth formula, victory, $60K colony-failure threshold, "First Founder" title), §19.3.5/13/14 (Summary, Colony Failure, Victory screen specs), §19.2 (16-color palette), §2 (round counts: 6/12/12). Rewrote [`game/scoring.md`](game/scoring.md) with: full per-commodity formula, pre-auction starting-price fallback, Summary row content (color swatch + species + net worth + land count + rank), Victory tie-rule ("all tied players are winners" per §17.2), Colony Failure trigger + screen, mode-gated survival (Beginner skips), tie-break = lower index, and a flag that `main.c` hardcodes `max_rounds=6` regardless of mode. Requirements block delivered to coding-architect for `R_OP_TEXT` opcode design + 7a module split. No GDD source page rewrite needed — §8/§17/§19 were already marked ingested in [`sources/gdd.md`](sources/gdd.md); this pass deepened the scoring.md derivation.

---

## [2026-04-30] decision | Phase 6.5 closed — no CoWin text on CoVDG; HUD = render-child glyphs

Two-PoC investigation. [`poc_owtxt`](../src/c/poc_owtxt.c) (PR #48) ran six visual probes — all six negative. Diagnosis: the parallel-CoWin probe was malformed (opened parent `/w` instead of an `iniz`'d numbered `/wN`), so Q1b wasn't truly answered, and Q2/Q3 were routed through the broken paths. [`poc_owtxt2`](../src/c/poc_owtxt2.c) (PR #49 + user-committed source) ran three tighter probes on EOU 2026-04-30: **P1 fixed parallel CoWin window** (`iniz /w1`/`/w2`/`/w7`/`/w8` + DWSet `STY=$FF`) — **no**, all paths open cleanly but DWSet is silently swallowed, no window appears on a CoVDG-allocated screen; **P2 palette escape** (`ESC $31 PRN CTN` to the VDG path) — **yes**, repaints palette registers system-wide, resolves the deferred Phase 4 `R_OP_PALETTE`; **P3 bitmap-glyph demo** (8×8 bit-packed "HELLO" via `putpx`) — **yes**, demonstrates the option-2 fallback. **Architectural conclusion:** there is no escape-driven text path onto a CoVDG-allocated screen. Phase 7a HUD/score/prompts render as render-child-owned bitmap glyphs through a new `R_OP_TEXT(x, y, str, color)` command; `R_OP_PALETTE` becomes a 4-byte escape write to `g_path`. Updated [`platform/cowin-text.md`](platform/cowin-text.md) (replaced "Open questions" with "Resolved 2026-04-30"; updated OWSet caveat and Palette section), [`implementation/lessons-learned.md`](implementation/lessons-learned.md) (two new sections: "CoWin text on CoVDG screens — not possible" and "Palette change on a CoVDG screen — use the escape, not a SetStat"), [`implementation/poc-catalog.md`](implementation/poc-catalog.md) (added `poc_owtxt` and `poc_owtxt2` entries, resolved the render.c deferred note), [`implementation/roadmap.md`](implementation/roadmap.md) (row 6.5 marked ✅ with finding summary). Phase 7a is now unblocked.

---

## [2026-04-26] decision | Phase 6 (Minimal AI) dissolved into 7a–7g

Starting "phase 6" surfaced a problem: the original plan was a standalone AI module landing before any playable phase, but with 7a–7g not yet built, an AI module would have to guess at `GameState` shape and phase entry hooks with no consumer to validate against — exactly the speculative-abstraction shape we avoid. Decided path (c) of three options: dissolve phase 6, ingest GDD §18 now as the per-phase spec, and let each AI policy ship inside its consuming 7x phase (land-grant in 7b, management in 7c/7d, auction in 7e/7f; 7a needs none). 7a is unblocked by this — its only remaining gate is 6.5 (`poc_owtxt`). Substantively ingested GDD §18 (lines 976–1006) into [`game/ai.md`](game/ai.md): turn-priority list, `EV = terrain_value*remaining_rounds + 500` land-grant formula, seller/buyer auction stance + timer-driven price walk, MULE-buy + outfit-priority rules, and the +$200 Tournament starting-cash bonus. Updated [`sources/gdd.md`](sources/gdd.md) to mark §18 ingested, [`implementation/roadmap.md`](implementation/roadmap.md) to strike row 6 / fix the dependency graph / shorten the critical path to ~12–13 weeks, [`implementation/poc-catalog.md`](implementation/poc-catalog.md) Next-pointer to phase 6.5, and [`index.md`](index.md). Risk accepted and recorded: if a later 7x phase needs cross-policy reads (e.g. auction-AI consulting management-AI for cash reserves), a small shared `ai.c` is factored out at that point.

---

## [2026-04-26] ingest | NitrOS-9 EOU Level 2 Windowing System Manual — Tier 1

Strategic-assessment query split the 1385-line manual into Tier 1 (text/overlay path, now), Tier 2 (drawing primitives, JIT before 7f auctions), Tier 3 (skip — arcs/ellipses/flood fill/font scaling). Tier 1 sections ingested: Ch.1 Device/Overlay window concepts (101–187); Ch.3 DWSet (506–555), OWSet/OWEnd (797–831), Palette (833–869), DefColr (305–335); Ch.4 optimization preamble (1095–1107); Ch.5 text commands + optimization tips (1339–1384). Created [`platform/cowin-text.md`](platform/cowin-text.md) covering window kinds, DWSet screen-type table (incl. GIME 199-line bug), OWSet save-switch tradeoffs, Palette 6-bit RGB encoding (per-screen scope), Ch.5 text command bytes, font speed caveats (8-px > 6-px > proportional, type-5 screen fastest for single-color text), and 16-pixel alignment rule for type-8. Recorded three open questions blocking phase 7a: (1) does OWSet accept the render child's CoVDG path, (2) Palette vs SS.PalSet on a CoVDG-displayed screen, (3) standard-font preload state under `pioneer` — proposing a small `poc_owtxt` before 7a Summary/Scoring. Documented Palette escape sequence as the simpler path forward for the deferred `R_OP_PALETTE`. Linked from [`platform/cowin.md`](platform/cowin.md) and [`index.md`](index.md); updated source stub [`sources/nitros9-docs.md`](sources/nitros9-docs.md) to mark Tier 1 ingested and Ch.4 deferred.
