...
- **DCC does not promote `char` to `int` for varargs.** Passing an `unsigned char` directly to `printf("%d", ...)` produced a system-trashing crash on the first call ("graphic bars down the screen" — printf reads 2 bytes per `%d`, gets stack garbage in the high byte, terminal interprets the resulting high-bit/control bytes as block-graphic glyphs and fills the screen). **Always cast `(int)` at the printf call site** when the value is `char`/`unsigned char`. Every working PoC in `src/c/` already passes `int` args (`vcnt`, `r.rg_b & 0xff`, etc.) — `main.c` was the first place we passed raw `unsigned char` and it crashed immediately. Observed 2026-04-25 running `pp` on EOU.
