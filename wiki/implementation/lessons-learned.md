...
- **`direct` storage class crashes user processes.** Declaring `direct GameState g_state` in [main.c](../../src/c/main.c) caused a NitrOS-9 system crash on first `printf` (graphic bars down the screen — runaway code or corrupted MMU state). Hypothesis: DCC's `direct` allocator overlaps libc's own direct-page slots (`_flacc`, `errno`, ...; see [SLPICPT.c:71](../../../../stocks-and-bonds/src/c/SLPICPT.c)). Until a dedicated PoC validates safe direct-page usage (e.g. via an explicit `vsect`/`endsect` reservation that coexists with the runtime), **avoid `direct` in production code.** Observed 2026-04-25 running `pp` on EOU. No existing PoC in `src/c/` uses `direct`.
