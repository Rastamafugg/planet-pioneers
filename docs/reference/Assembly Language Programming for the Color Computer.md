...
= Memory Map Mode - This bit, TY, is cleared by BASIC when the computer is
...
m= MPU Clock Rate - Control bits RO and R1 control the MPU clock signals, E
...
clock rate is also dependent on what addresses are being accessed by the

MPU.

et RO | Clock Rate While Addressing
R1 RO | 0-7FFF] 8000-FEFF | FFOO-FFIF | FF20-FFFF
...
m Page Number - The page number bit, Pl, is cleared by BASIC when the
...
mw Video Display Starting Address - The video display starting address, bits
...
F6 F5 F4 F3 F2 Fl FO 0 00000000
...
m Video Display Mode - The video display mode bits, VO, V1, and V2, are
...
shown in Table 9-4.
V2 Vi1.VO Buffer Size Display Modes_
AI,SG4,SG6
...
u Decoding with TY Clear - When the TY bit is clear, the addresses are
...
Dedicated Addresses |
FF04,FF08,FFOC,FFI0,FF14,FF18,FFIC
FF05,FF09,FFOD,FF11,FF15,FF19,FFID
FF06,FFOA,FFOE,FF12,FF16,FFIA,FFIE
FFO7,FFOB,FFOF,FF13,FF17,FFIB,FFIF
FF24,FF28 ,FF2C,FF30,FF34,FF38,FF3C
FF25,FF29,FF2D,FF31,FF35,FF39,FF3D
FF26,FF2A,FF2E,FF32,FF36,FF3A,FF3E
FF27,FF2B,FF2F,FF33,FF37,FF3B,FF3F

Table 9-5 PIA Primary and Redundant Addresses (TY=0).

Table 9-6 Disk Controller Primary and Redundant Addresses (TY=0).

m Decoding With TY Set - When the TY bit is set, the addresses are decoded
...
2 are shown in Table 9-5. The

disk controller can be accessed only through
...
Control Data Direction Register B (DDRB) Data Path B
...
corresponding bit of the DRA: DDRAO controls DRA0, DDRAI controls DRAI,
...
=m Ls
Flag Flag CA2(CB2) Control {DDR/DR CAI1(CB1)
CA(B)1 | CA(B)2 Access Control

Fig. 9-5 PIA Control Register A(B).
Courtesy of Motorola, Inc.

m CAI(CB1) Control - The CA1 control field controls how the CAI signal is
...
m CA2 Control - The CA2 signal can be configured as an input or an output.
...
= CB2 Control - The CB2 signal can be configured as an input or an output.
In the Color Computer it is used for output only. Bits 3, 4, and 5 of the
...
m PIA Dedicated Addresses - Each PIA can be accessed by four primary
...
= Initializing the PIAs - The PIAs are initialized by BASIC when the
...
gw The Interrupt Flags - The interrupt flags are bits 6 and 7 of the control
...
VDG Op Mode |SAM Control Bits BASIC Display
16543 v2 v1 vo Mode PMODE | SCREEN
...
a0Aa000.0 00%
ae OOO OK FO COO
ee OOF F COON; OKR COO
POorororroaocococ0d

J~
don't care | e=color set (0 or 1)
...
270 STA SFF23 PUT IN CRB
280 LDA #SF8 DDRB CONFG
...
130 RRO IO III IO III IR III

140. PM1 STA $SFFC7 SET FO BIT
...
Byte 1 [i Ny N, Nj) Py Ps Py, Py

Byte 32] 1 No Ny No Peg Pes Peg Por
...
Display (top left corner) purer Contents

Byte 0/1 N Po Py
Byte 1.]1.°N Pe, Pr Pe ns rhs i
...
Byte 32} 1 N- Piog Pyo9 Pano P2o1 P2o2 P203

202

Color Set: C=0 N P.. C=1 N P,
x 0 - Black x O - Black
...
Po {Px {Po [Ps] - - Byte 0 |1 Ny Ny; Ny Po Py X X
PealPes | Byte 1 [i N, N, N, P,P, xX
Byte 32 [I N,N, No Pe, Pe, X X
...
BASIC Display Mode: Not supported.
Border Color: If C=0, then green. Buffer Size: 1024
If C=1, then buff.
Display Format: 64 columns, 64 rows
Display (top left corner) Buffer Contents

repre . Byte 0 1Nig Noo Nay Nor Nie Nog Nig No3
Pez . .

Byte 16 | Niga Noga Nags Noes Niee Nose Niez7 Noe

Color Set: C=0 N, N, C=1 N, N,
0 O - Green 0 O - Buff
0 1- Yellow QO 1 - Cyan
1 O- Blue 1 0 - Magenta
1 1 - Red 1 1 - Orange
...
Byte 0 0 Py Po P3 P, Py Pe Py

Byte 16 Figg Pi29 P1309 Pigi Pige Pigs Pisa Piss
...
Byte 32 [N,,..N,.o,Nii0, N

1128 Noi2g Ni1290 No129 Ni130 No130 Nii31 Nois
Color Set: C=0 N; No C=] N; No
0 O- Green 0 O- Buff
0 1 - Yellow QO 1 - Cyan
1. 0 - Blue 1 O - Magenta
1 1- Red 1.1 - Orange

Description: The screen is divided into 8192 pixels; each is controlled by
two bits of a byte in the buffer. One byte controls four pixels. The bits
Nj x and No, determine the color of their pixel, P,. All pixels are
...
Byte 16 | Prog Piao P1g0 Pigi Pige Pigg Pisa Pigs
...
Po [Pi [Po [Ps ]- - Byte 0 | Nig Noo Nai Nor Nye Nog Nis Nos
Pig lPi20]Pisol Pisa] - : .
Byte 32 |Ni123 Noz2s Nizz9 No129 Niz30 Noiz0 Niisi Noisi

Color Set: C=0 Ny No C=] N N

0. 0 - Green 0 0 0 Buff
0 1 - Yellow 0 1 - Cyan
1 O - Blue 1 O - Magenta
1. 1 - Red 1. 1 - Orange

Description: The screen is divided into 12288 pixels; each is controlled by
two bits of a byte in the buffer. One byte controls four pixels. The two
bits N;, and Nox determine the color of their pixel, P,. All the
...
Byte 32. | Nyi99 No92 Ni193 No193 Ni194 Noro Nizos Noios

Color Set: C=0 Ny No C=1_:‘Ny N
...
Description: The screen is divided into 24576 pixels; each is controlled by
two bits of a byte in the-buffer. One byte controls four pixels. The two
bits Nyx and Nox determine the color of their pixel, P,. All the
...
ase)? asa aso) ac]? aml Pasn aes a
Buffer Contents

Byte 0 Po P; P, Ps P, P; Pg Py
Byte 32 Pasa? 257 P 258 P59 Pa6o P261 Paes Paes
P
...
150 *CORNER.
OO RRR I KARR IRA ARAKI KI KIK IKE RRR RRR R RIKI RIRIRIR

170 GRPH — STA $FFC8 CLEAR F1

180 STA $FFCD SET F3

190 STA SFFC5 SET V2

200 STA $FFC3 SET V1

210 LDA #$F0 VDG OP MODE

220 STA SFF22 SET VDG OP MODE
230 CLRA GET 00
...
280 LDX #81104 START OF TOP LINE
290 LDA #SFF BUFFER CODE

300 GRB STA ,X+ PUT IN BUFFER
310 CMPX #31110 END OF LINE?
...
350 CMPX #51610 END OF LINE?

360 BLS GRC IF NOT,DO AGAIN

370 LDA #380 LEFT LINE CODE

380 LDX #51124 START OF LEFT LINE
390 GRD STA ,X PUT IN BUFFER

400 LEAX 32,X GOTO NEXT LOWER PIXEL
410 CMPX #51604 END OF LINE?
...
BOD RRR RII IR IIR III RII RII RTI I IRI TR II
510 END
...
8 ROM Cartridge Slot 44

Fig. 9-6 Block Diagram of the Interrupt Sources.

= IRQ Interrupts - PIA 1 is connected to the IRQ interrupt signal going to
...
m FIRQ Interrupts - PIA 2 is connected to the FIRQ interrupt signal going
...
mu NMI Interrupts - The NMI interrupt signal line to the MPU is connected
...
STA SFFO3 STORE IN CRB

LDA $FFO2 READ DRB(CLR FLAG BIT)
...
LDB $FFO2 READ DRB(CLR FLAG BIT)
RTI RETURN
...
LDA $FFO2 ‘READ DRBC(CLR FLAG)
ANDCC #SEF CLEAR I BIT

RMB 30 STACK AREA

INT. HANDLER

LDA SFFO2 READ DRBCCLR FLAG)
RTI RETURN
...
A
A/D converter, 264-266 characteristic, 33-35, 41-43
absolute address, 135 CHROUT, 200-202
accumulator, 54-55 CLEAR, BASIC, 188
accumulator offset, 75-76 CLSCRN, 199-200
addition, binary, 7-9 comparator, 262
address bus, 49, 270 complement, 14-15
ALU, 51 conditional branch, 54
analog-to-digital converter, 264-266 constant offset, 74-75
AND, 15-16 control bus, 49, 270
arithmetic logic unit, 51 converting base of number, 20-22
arrays, 44-46 copy, editor, 130-131
ASCII, 35-36 CSRDON, 213
ASCII dsiplay, 148-149 D
assembler listing, 134 D/A converter, 260-262
assembler options, 137-138 data block, 209-210
attenuator, 262-263 data bus, 49, 269
auto-decrement, 55, 76-77 decimal adjust, 31, 101
auto-increment, 55, 76-77 decoding addresses, 229-230
B DEFUSRn, BASIC, 192-193
BASIC variables, 41 delete, editor, 131
Baud, 201 digital-to-analog converter, 260-262
BCD, expanded, 31-32 direct addressing, 80
BOD, packed, 30-31 directory, 39-40
BLKIN, 213-216 directory entries, 218-219
BLKOUT, 212 disassemble, 147
Boolean algebra, 13-19 disk directory, 218-219
breakpoint, 152 disk I/O, 217-219
buffer, 128 DISPL, 202-203
byte, 19 display, ZBUG, 154
byte display, 148 division, binary, 13-14
byte overflow, 79 DSKCON, 219-223
Cc duality, 18
call, 164 E
main program responsibilities, 165-171 EBCDIC, 35
cartridge connector, 268-271 edit, 131
...
exclusive or, 16
EXEC, BASIC, 188-192
extended addressing, 68-69
extended indirect addressing, 69
F
FCB, 140
FCC, 140-141
FDB, 141
FIFO, 40
file allocation table, 218-219
find, 131-132
FIRQ interrupt, 63-64, 122, 250-251
fixed point, 32-33
flags, CC, 52-53
floating point, 33-35, 41-43
G
“Go, 151
granule, 218
H
half-carry, 53
HALT, 60, 270
header block, 208-209
hexadecimal numbers, 19-23
I
immediate addressing, 67-68
inclusive or, 15
index registers, 55-56
indexed addressing, 72-77
indexed indirect addressing, 77-78
inherent addressing, 67
insert, 132
instruction fetch, 50-51
interrupts, controlling, 249-252, 257
interrupt handler, 176, 181-185
interrupt vector table, 177
IRQ interrupt, 62-63, 122-123, 250
J
JOYIN, 206-207
joystick fire button, 205, 256-257
joystick position, 205-207, 264-266
K
keyboard, 203-205, 255
L
...
M
machine instruction, 47-48
machine language, 48
mantissa, 33-35, 41-43
memory, 24-27
memory size, 227, 258-259
mnemonic, 48
mnemonic display, 147
move, ZBUG, 152-153
MPU controller, 49-50
MPU cycle rate, 227-228
multiplication, binary, 12-13
multiprogramming, 175

N
name, 48
nested subroutines, 166-167
nibble, 20
NMI interrupt, 64, 123-124, 253
normalize, 33-35
NOT, 14-15
numeric base, 149
numeric variable, 41-42
...
P
Page #, 228
PC relative addressing, 80-83
Peripheral Interface Adapter, 230-232
PIA, 230-232
PIA control register, 233-235
PIA data direction register, 232-233
PIA data register, 232
PIA dedicated addresses, 235-236
...
pointer, 37-41
POLCAT, 203-205
position independent, 81-82
postbyte, 67
pre-interrupt responsibilities, 179-181
print, editor, 132
print, ZBUG, 154
printer, editor, 132
pseudo-commands, 139-143
pseudo-operation, 127
pull, 41
push, 41
put, ZBUG, 153
Q
...
R
RAM, 24
reentrant subroutines, 176
redundant dedicated addresses, 229-230
register display, 151-152
registers; A, B, D, 54-55
CO, 52-54
DP, 57-58
I, 49
PC, 50-51
U, S, 56-57
X, Y, 55-56
register addressing, 70-72
relative address, 136
...
Ss
SAM, 225-230
SAM dedicated addresses, 225
sector, 218
selector switch, 262
serial I/O, 201-202, 257-258
SET, 142-143
SETDP, 143
sign extension 7, 116

signed binary, 5-7, 92-30
single step, 151
skip factor, 218
software interrupts, 64-65
sound generation, 266-267
stack, 40-41
stack pointer, 56
straight binary, 4-5, 29
string descriptor, 43-46
string variable, 43-44
subroutine, 164-165
subtraction, binary, 10-12
symbol, 135
synchronous address multiplexer, 225-230
system bus, 49

T
tables, 36-39
track, 217
two’s complement, 5-6, 9-10
TY bit, 225-226
...
Vv
VDG, 236-238
vector address, 61, 177
vector jump table, 177
verify, editor, 133
verify, ZBUG, 153
video display code, 144, 239
video display generator, 236-238
video display modes, 237, 239-249
video starting address, 228, 236
volatile, 24
...
was written on the Color Computer using the Telewriter-64
(Cognetic) word processing program. Also, every word of text
was typeset using the CoCo and the Hewlett Packard LaserJet
printer. The possible uses of the CoCo seem limitless.

This book shows in a tutorial fashion how to program in
assembly language. Assembly language may be low level but it
provides access to all the CoCo capabilities and programs
written in this language execute very quickly. Therefore,
all the hardware functions and capabilities of the CoCo and
how to control them with assembly language are also
explained in this book. A knowledge of assembly language
will allow you to write your own unique programs. This
knowledge will also allow you to determine how other
programs work and then modify them te suit your needs.