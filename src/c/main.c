 ...
 *
 * Minimal turn-phase state machine. Each phase is a printf inlined in
 * the switch (no helper functions, to keep DCC's symbol/codegen path
 * simple). All GameState fields are int -- size-not-type ABI on
 * NitrOS-9 means we should not pass char-typed values to varargs
 * unless we widen them, so we just store as int from the start.
 *
