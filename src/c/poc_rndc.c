...
/* 8x8 bitmap font, 96 entries (ch 0x20..0x7F). Glyph occupies bits 1..6
 * of each row (cols 0,7 are spacing). Inlined here rather than #included
 * because DCC's `#include "..."` falls through to /dd/defs and the build
 * deploy doesn't ship .h files alongside .c (lessons-learned 2026-05-01).
 *
 * static const => code segment, zero data-segment cost. */
static const unsigned char g_font[96][8] = {
    ...
    /* 0x61..0x7a 'a'..'z' lowercase: blank for now */
